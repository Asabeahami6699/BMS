...
export function InitiateAgencyWithdrawalPanel({ onInitiated }: Props) {
  const { user } = useAuth();
  ...
  return (
    <section className="card workspace-animate-in agency-withdrawals-initiate">
      <header className="agency-withdrawals-initiate__head">
        <div>
          <h3>Initiate withdrawal</h3>
          <p className="muted">
            Collect customer details and start the withdrawal — walk-in non-BMS requests go straight to
            teller; BMS members are verified in the queue below.
          </p>
        </div>
      </header>
