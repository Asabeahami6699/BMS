...
export function TransactionPinProvider({ children }: { children: ReactNode }) {
  ...
  const [blockedRequiresAdminReset, setBlockedRequiresAdminReset] = useState(false);
  ...
  const applyPinStatus = useCallback((status: Awaited<ReturnType<typeof getTransactionPinStatus>>) => {
    ...
    setBlockedRequiresAdminReset(Boolean(status.blockedRequiresAdminReset));
  ...
  async function handleSubmit(pin: string, confirmPin?: string) {
    ...
    try {
      ...
      setError(message);
      const blocked =
        message.toLowerCase().includes("locked") && message.toLowerCase().includes("administrator");
      if (blocked) {
        setBlockedRequiresAdminReset(true);
        clearCachedTransactionStepUpToken();
      }
    } finally {
      setBusy(false);
  ...
  return (
    <TransactionPinContext.Provider value={{ requestStepUp }}>
      ...
      <TransactionPinModal
        ...
        blockedRequiresAdminReset={blockedRequiresAdminReset}
        error={error}
        busy={busy}
        onClose={() => {
          if (resetRequired && !blockedRequiresAdminReset) {
